export const mockData = {
  "categories": [
    {
      "id": "ai-services",
      "name": "AI 与 智能服务",
      "icon": "🤖",
      "order": 1,
      "sites": [
        {
          "id": "dify",
          "name": "Dify AI",
          "url": "https://dify.yunpiao.site",
          "description": "AI 应用开发与编排平台",
          "icon": ""
        },
        {
          "id": "any2text",
          "name": "Any2Text",
          "url": "https://any2text.yunpiao.site",
          "description": "多模态转文本服务",
          "icon": ""
        },
        {
          "id": "notebooklm",
          "name": "Google NotebookLM",
          "url": "https://notebooklm.google.com/",
          "description": "智能笔记与研究助手",
          "icon": ""
        },
        {
          "id": "glm",
          "name": "智谱清言",
          "url": "https://chatglm.cn/",
          "description": "GLM-4 国产大模型",
          "icon": ""
        },
        {
          "id": "tts",
          "name": "TTS",
          "url": "https://tts.yunpiao.site",
          "description": "语音合成服务",
          "icon": ""
        }
      ]
    },
    {
      "id": "dev-ops",
      "name": "监控与运维",
      "icon": "📊",
      "order": 2,
      "sites": [
        {
          "id": "grafana",
          "name": "Grafana",
          "url": "https://grafana.yunpiao.site/dashboards/",
          "description": "监控数据可视化大屏",
          "icon": ""
        },
        {
          "id": "grafana-dashboard-generator",
          "name": "Grafana Dashboard Generator",
          "url": "https://grafana-dashboard-generator.yunpiao.site/",
          "description": "Grafana 仪表盘生成器",
          "icon": ""
        },
        {
          "id": "kibana",
          "name": "Kibana",
          "url": "https://kibana.yunpiao.site",
          "description": "日志分析与搜索看板",
          "icon": ""
        },
        {
          "id": "prom",
          "name": "Prometheus",
          "url": "https://prom.yunpiao.site",
          "description": "指标监控与报警系统",
          "icon": ""
        },
        {
          "id": "jaeger",
          "name": "Jaeger",
          "url": "https://jaeger.yunpiao.site",
          "description": "分布式链路追踪",
          "icon": ""
        },
        {
          "id": "v-panel",
          "name": "V Panel",
          "url": "https://v.yunpiao.site",
          "description": "主服务器管理面板",
          "icon": ""
        },
        {
          "id": "v1-panel",
          "name": "V1 Panel",
          "url": "https://v1.yunpiao.site",
          "description": "备用服务器管理面板",
          "icon": ""
        },
        {
          "id": "kuma",
          "name": "Uptime Kuma",
          "url": "https://kuma.yunpiao.site",
          "description": "服务可用性监控",
          "icon": ""
        },
        {
          "id": "cf-dash",
          "name": "CF Dashboard",
          "url": "https://cf-dash.yunpiao.site",
          "description": "Cloudflare 流量仪表盘",
          "icon": ""
        }
      ]
    },
    {
      "id": "data-storage",
      "name": "存储与数据",
      "icon": "💾",
      "order": 3,
      "sites": [
        {
          "id": "obs",
          "name": "OBS 存储",
          "url": "https://obs.yunpiao.site",
          "description": "对象存储服务 (MinIO/OBS)",
          "icon": ""
        },
        {
          "id": "couch",
          "name": "CouchDB",
          "url": "https://couch.yunpiao.site",
          "description": "分布式文档数据库",
          "icon": ""
        },
        {
          "id": "rsshub",
          "name": "RSSHub",
          "url": "https://rsshub.yunpiao.site",
          "description": "万物皆可 RSS 生成器",
          "icon": ""
        },
        {
          "id": "rss",
          "name": "RSS Reader",
          "url": "https://rss.yunpiao.site",
          "description": "个人 RSS 订阅阅读器",
          "icon": ""
        }
      ]
    },
    {
      "id": "tools",
      "name": "实用工具",
      "icon": "🛠️",
      "order": 4,
      "sites": [
        {
          "id": "tools",
          "name": "Tools Box",
          "url": "https://tools.yunpiao.site",
          "description": "云飘工具箱",
          "icon": ""
        },
        {
          "id": "dec",
          "name": "DEC Service",
          "url": "https://dec.yunpiao.site",
          "description": "数据解码/处理服务",
          "icon": ""
        },
        {
          "id": "ts",
          "name": "TS Service",
          "url": "https://ts.yunpiao.site",
          "description": "传输/同步服务",
          "icon": ""
        },
        {
          "id": "fenshululu",
          "name": "分数录录",
          "url": "https://www.fenshululu.cn/",
          "description": "分数录入系统",
          "icon": ""
        },
        {
          "id": "fenshululu-chat",
          "name": "分数录录聊天",
          "url": "https://chat.fenshululu.cn/",
          "description": "分数录录在线客服系统",
          "icon": ""
        },
        {
          "id": "noise",
          "name": "Noise Service",
          "url": "https://noise.yunpiao.site/",
          "description": "白噪音与环境音服务",
          "icon": ""
        },
        {
          "id": "newapi",
          "name": "NewAPI",
          "url": "https://newapi.yunpiao.site",
          "description": "API 管理服务",
          "icon": ""
        }
      ]
    },
    {
      "id": "personal",
      "name": "个人空间",
      "icon": "🏡",
      "order": 5,
      "sites": [
        {
          "id": "blog",
          "name": "云飘的博客",
          "url": "https://blog.yunpiao.site/",
          "description": "记录生活与技术",
          "icon": ""
        },
        {
          "id": "github",
          "name": "GitHub",
          "url": "https://github.com/yunpiao",
          "description": "代码托管与开源项目",
          "icon": ""
        },
        {
          "id": "umami",
          "name": "Umami",
          "url": "https://cloud.umami.is/analytics/us/websites/b341aa93-c0f2-40b4-bee8-dff72e443562?date=7day",
          "description": "访问统计",
          "icon": ""
        },
        {
          "id": "happy-web",
          "name": "Happy Web",
          "url": "https://happy-web.yunpiao.site",
          "description": "Happy Web 站点",
          "icon": ""
        }
      ]
    }
  ],
  "title": "MAO NAV"
}
